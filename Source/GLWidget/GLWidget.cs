...
namespace Gtk {
	...
	public class GLWidget: DrawingArea, IDisposable {
		...
		IWindowInfo InitializeWindows() {
			...
			return OpenTK.Platform.Utilities.CreateWindowsWindowInfo(windowHandle);
		}
		...
		IWindowInfo InitializeOSX() {
			...
			return OpenTK.Platform.Utilities.CreateMacOSWindowInfo(windowHandle, viewHandle);
		}
		...
		IWindowInfo InitializeX(GraphicsMode mode) {
			...
			retval = OpenTK.Platform.Utilities.CreateX11WindowInfo(display, screen, windowHandle, rootWindow, visualInfo);
			XFree(visualInfo);
