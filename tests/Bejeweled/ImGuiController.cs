...
{
    vec3 srgb = srgba.rgb;
    vec3 rgb = mix(pow((srgb + vec3(0.055)) / vec3(1.055), vec3(2.4)), srgb / 12.92, lessThanEqual(srgb, vec3(0.04045)));
...
void main()
{
    ...
    outputColor = srgb_to_linear(outputColor);
}";
...
void main()
{
    outputColor = color * texture(in_fontTexture, texCoord);
        ...
        internal void RenderImDrawData(ImDrawDataPtr draw_data)
        {
