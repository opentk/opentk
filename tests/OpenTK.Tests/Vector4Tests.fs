...
module Vector4 =
    ...
    module Multiplication =
        ...
        [<Property>]
        let ``Vector4-Matrix4x2 multiplication is the same as vector/column multiplication and summation`` (a : Matrix4x2, b : Vector4) =
            let res = b * a
        ...
        [<Property>]
        let ``Vector4-Matrix4x2 multiplication is consistent across overloads`` (a : Matrix4x2, b : Vector4) =
            let r1 = b * a;
            let r2 = Vector4.TransformTwoDimensionsRow(b, a);
            let r3 = Vector4.TransformTwoDimensionsRow(&b, &a);
        ...
        [<Property>]
        let ``Matrix2x4-Vector4 multiplication is the same as vector/column multiplication and summation`` (a : Matrix2x4, b : Vector4) =
            let res = a * b
        ...
        let ``Matrix2x4-Vector4 multiplication is consistent across overloads`` (a : Matrix2x4, b : Vector4) =
            let r1 = a * b;
            let r2 = Vector4.TransformTwoDimensionsColumn(a, b);
            let r3 = Vector4.TransformTwoDimensionsColumn(&a, &b);

            Assert.Equal(r1, r2)
            Assert.Equal(r2, r3)

        [<Property>]
        let ``Vector4-Matrix4x3 multiplication is the same as vector/column multiplication and summation`` (a : Matrix4x3, b : Vector4) =
            let res = b * a
        ...
        [<Property>]
        let ``Matrix3x4-Vector4 multiplication is the same as vector/column multiplication and summation`` (a : Matrix3x4, b : Vector4) =
            let res = a * b
        ...
        let ``Matrix3x4-Vector4 multiplication is consistent across overloads`` (a : Matrix3x4, b : Vector4) =
            let r1 = a * b;
            let r2 = Vector4.TransformThreeDimensionsColumn(a, b);
            let r3 = Vector4.TransformThreeDimensionsColumn(&a, &b);
    ...
    module Interpolation =
        ...
        [<Property>]
        let ``Component-wise linear interpolation works like regular linear interpolation`` (a : Vector4, b : Vector4, q : Vector4) =
            let blend = q
            
            let expectedX = Vector4.Lerp(a, b, blend.X).X
            let expectedY = Vector4.Lerp(a, b, blend.Y).Y
            let expectedZ = Vector4.Lerp(a, b, blend.Z).Z
            let expectedW = Vector4.Lerp(a, b, blend.W).W
            let expected = Vector4(expectedX, expectedY, expectedZ, expectedW)
            
            Assert.Equal(expected, Vector4.Lerp(a, b, q))
            Assert.Equal(expected, Vector4.Lerp(&a, &b, q))
        ...
        let ``Lerp returns enpoints`` (a : Vector4, b : Vector4) = 
            let c = Vector4.Lerp(a, b, 0.0f)
            let d = Vector4.Lerp(a, b, 1.0f)
        ...
        let ``Elerp returns enpoints`` (a : Vector4, b : Vector4) = 
            let c = Vector4.Elerp(Vector4.Abs(a), Vector4.Abs(b), 0.0f)
            let d = Vector4.Elerp(Vector4.Abs(a), Vector4.Abs(b), 1.0f)
            Assert.ApproximatelyEquivalent(Vector4.Abs(a), c);
            Assert.ApproximatelyEquivalent(Vector4.Abs(b), d);
