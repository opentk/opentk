...
module Vector2 =
    ...
    module Length =
        ...
        let ``Fast length method works`` (a, b) =
            let v = Vector2(a, b)
            let l = 1.0f / MathF.ReciprocalSqrtEstimate(a * a + b * b)
    ...
    module Multiplication =
        ...
        let ``Vector2-Matrix2x3 multiplication is consistent across overloads`` (a : Matrix2x3, b : Vector2) =
            let r1 = b * a;
            let r2 = Vector2.TransformToThreeDimensions(b, a);
            let r3 = Vector2.TransformToThreeDimensions(&b, &a);
        ...
        let ``Vector2-Matrix2x4 multiplication is consistent across overloads`` (a : Matrix2x4, b : Vector2) =
            let r1 = b * a;
            let r2 = Vector2.TransformToFourDimensions(b, a);
            let r3 = Vector2.TransformToFourDimensions(&b, &a);
    ...
    module Interpolation =
        ...
        [<Property>]
        let ``Component-wise linear interpolation works like regular linear interpolation`` (a : Vector2, b : Vector2, q : Vector2) =
            let blend = q
            
            let expectedX = Vector2.Lerp(a, b, blend.X).X
            let expectedY = Vector2.Lerp(a, b, blend.Y).Y
            let expected = Vector2(expectedX, expectedY)
            
            Assert.Equal(expected, Vector2.Lerp(a, b, q))
            Assert.Equal(expected, Vector2.Lerp(&a, &b, q))
    ...
    module Normalization =
        ...
        let ``Fast approximate normalization of instance transforms the instance into a unit length vector with the correct components`` (a, b) =
            ...
            let scale = MathF.ReciprocalSqrtEstimate(a * a + b * b)
        ...
        let ``Fast approximate normalization by reference is the same as multiplication by the fast inverse square`` (a : Vector2) =
            let scale = MathF.ReciprocalSqrtEstimate(a.X * a.X + a.Y * a.Y)
