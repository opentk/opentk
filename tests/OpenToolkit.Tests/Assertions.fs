...
type internal Assert =
    ...
    static member ApproximatelyEquivalentEpsilon(a : Vector2, b : Vector2, epsilon:float32) =
        if neqEpsilon a.X b.X epsilon || neqEpsilon a.Y b.Y epsilon then
            raise <| new Xunit.Sdk.EqualException(a,b)

    static member ApproximatelyEquivalentEpsilon(a : Vector3, b : Vector3, epsilon:float32) =
        if neqEpsilon a.X b.X epsilon || neqEpsilon a.Y b.Y epsilon || neqEpsilon a.Z b.Z epsilon then
            raise <| new Xunit.Sdk.EqualException(a,b)

    static member ApproximatelyEquivalentEpsilon(a : Vector4, b : Vector4, epsilon:float32) =
