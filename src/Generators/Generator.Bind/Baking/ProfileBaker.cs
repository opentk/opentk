...
namespace Bind.Baking
{
    ...
    public class ProfileBaker
    {
        ...
        )
        {
            ...
            MergeDuplicateEnumerations(translatedProfile);
        ...
        )
        {
            return functionOverride.NameType == OverrideNameType.EntryPoint
                ...
                : existingFunctions.Where(x => Utilities.GetNameVariations(functionOverride.Name).Contains(x.Name));
        }
            ...
            foreach (var profileOverride in profileOverrides)
            {
                foreach (var function in profileOverride.RemovedFunctions)
                {
                    if (removals.Any(f => Utilities.GetNameVariations(function).Contains(f.Name)))
                    {
                        ...
                        (
                            "Duplicate removal redefinition in overrides detected - generator error or schema change."
                        );
                    }
