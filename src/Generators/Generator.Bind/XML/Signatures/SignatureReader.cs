...
namespace Bind.XML.Signatures
{
    ...
    public static class SignatureReader
    {
        ...
        [NotNull]
        internal static FunctionSignature ParseFunctionSignature([NotNull] XElement functionElement)
        {
            ...
            var functionCategoriesWithoutDuplicates = new List<string>();
            foreach (var functionCategory in functionCategories)
            {
                if (functionCategoriesWithoutDuplicates.Contains(functionCategory))
                {
                    continue;
                }

                functionCategoriesWithoutDuplicates.Add(functionCategory);
            }

            functionCategories = functionCategoriesWithoutDuplicates.ToArray();
