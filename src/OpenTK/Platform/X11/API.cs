...
XF86VidModeGetGammaRampSize(
    ...
    int *size);
        ...
        internal struct XF86VidModeModeInfo
        {
            ...
            private readonly int privsize; /* Size of private */
            private readonly IntPtr _private; /* Server privates */
        }
        ...
        {
            [MarshalAs(UnmanagedType.LPStr)] private readonly string vendor; /* Name of manufacturer */
            [MarshalAs(UnmanagedType.LPStr)] private readonly string model; /* Model name */
            private readonly float EMPTY; /* unused, for backward compatibility */

            private readonly byte nhsync; /* Number of horiz sync ranges */

            /*XF86VidModeSyncRange* */
            private readonly IntPtr hsync; /* Horizontal sync ranges */

            private readonly byte nvsync; /* Number of vert sync ranges */

            /*XF86VidModeSyncRange* */
            private readonly IntPtr vsync; /* Vertical sync ranges */
        }

        [StructLayout(LayoutKind.Sequential)]
        internal struct XF86VidModeSyncRange
        {
            private readonly float hi; /* Top of range */
            private readonly float lo; /* Bottom of range */
        }

        [StructLayout(LayoutKind.Sequential)]
        internal struct XF86VidModeNotifyEvent
        {
            private readonly int type; /* of event */
            private readonly ulong serial; /* # of last request processed by server */
            private readonly bool send_event; /* true if this came from a SendEvent req */
            private readonly Display display; /* Display the event was read from */
            private readonly IntPtr root; /* root window of event screen */
            private readonly int state; /* What happened */
            private readonly int kind; /* What happened */

            private readonly bool forced; /* extents of new region */

            /* Time */
            private readonly IntPtr time; /* event timestamp */
        }

        [StructLayout(LayoutKind.Sequential)]
        internal struct XF86VidModeGamma
        {
            private readonly float red; /* Red Gamma value */
            private readonly float green; /* Green Gamma value */
            private readonly float blue; /* Blue Gamma value */
        }
    }
    ...
    internal static partial class Functions
    {
        ...
        internal static extern void XChangeWindowAttributes(Display display, Window w, UIntPtr valuemask,
            ref XSetWindowAttributes attributes);

        internal static void XChangeWindowAttributes(Display display, Window w, SetWindowValuemask valuemask,
            ref XSetWindowAttributes attributes)
        {
        ...
        */
        ...
        [DllImport(XrandrLibrary)]
        public static extern int XRRQueryVersion(Display dpy, ref int major_versionp, ref int minor_versionp);
        ...
        [DllImport(XrandrLibrary)]
        public static extern int XRRSetScreenConfig(Display dpy, XRRScreenConfiguration config,
            Drawable draw, int size_index, ushort rotation, Time timestamp);

        [DllImport(XrandrLibrary)]
        public static extern int XRRSetScreenConfigAndRate(Display dpy, XRRScreenConfiguration config,
            Drawable draw, int size_index, ushort rotation, short rate, Time timestamp);

        [DllImport(XrandrLibrary)]
        public static extern ushort XRRConfigRotations(XRRScreenConfiguration config, ref ushort current_rotation);
         ...
         */
        [DllImport(XrandrLibrary)]
        public static extern ushort XRRRotations(Display dpy, int screen, ref ushort current_rotation);
        ...
        public static short[] XRRRates(Display dpy, int screen, int size_index)
        {
            ...
            unsafe
            {
                var data = XRRRates(dpy, screen, size_index, &count);
                if (count == 0)
                {
                    return null;
                }
        ...
        [DllImport(X11Library)]
        public static extern unsafe Pixmap XCreateBitmapFromData(Display display, Window d, byte* data, int width,
            int height);

        [DllImport("libX11", EntryPoint = "XAllocColor")]
        public static extern int XAllocNamedColor(Display display, Colormap colormap, string color_name,
            out XColor screen_def_return, out XColor exact_def_return);
    }
    /*
    [StructLayout(LayoutKind.Sequential)]
            ...
            get
            {
                if (_display == IntPtr.Zero)
                {
                    throw new InvalidOperationException(
                        "Internal error (XLockDisplay with IntPtr.Zero). Please report this at https://github.com/opentk/opentk/issues");
                }
