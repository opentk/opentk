...
namespace OpenTK.Platform.X11
{
    ...
    internal sealed class X11GLContext : DesktopGraphicsContext
    {
            ...
            int major, int minor, GraphicsContextFlags flags)
        {
            ...
            var shareHandle =
                shared != null ? (shared as IGraphicsContextInternal).Context : (ContextHandle)IntPtr.Zero;
            ...
            if (fbconfig != IntPtr.Zero && major * 10 + minor >= 30 &&
                SupportsCreateContextAttribs(Display, currentWindow))
            {
            ...
            GraphicsContextFlags flags, ContextHandle shareContext)
        {
...

            {
                ...
                using (new XLock(display))
                {
                    context = Glx.Arb.CreateContextAttribs(display, fbconfig, shareContext.Handle, direct,
                        attributes.ToArray());
        ...
        public override void MakeCurrent(IWindowInfo window)
        {
            ...
            {
                Debug.Write(
                    $"Releasing context {Handle} from thread {Thread.CurrentThread.ManagedThreadId} (Display: {Display})... ");
            ...
            else
            {
                ...
                Debug.Write(
                    $"Making context {Handle} current on thread {Thread.CurrentThread.ManagedThreadId} (Display: {Display}, Screen: {w.Screen}, Window: {w.Handle})... ");
