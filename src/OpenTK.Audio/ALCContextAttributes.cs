...
namespace OpenTK.Audio.OpenAL
{
    ...
    /// </summary>
    public class ALCContextAttributes
    {
        /// <summary>
        ...
        public int[] CreateAttributeArray()
        {
            ...
            void AddAttribute(int? value, ALC.ContextAttribute attribute)
            {
                if (value != null)
                {
            ...
            AddAttribute(Frequency, ALC.ContextAttribute.Frequency);
            AddAttribute(MonoSources, ALC.ContextAttribute.MonoSources);
            AddAttribute(StereoSources, ALC.ContextAttribute.StereoSources);
            AddAttribute(Refresh, ALC.ContextAttribute.Refresh);
            if (Sync != null)
            {
                AddAttribute((Sync ?? false) ? 1 : 0, ALC.ContextAttribute.Sync);
            }
        ...
        public static ALCContextAttributes FromArray(int[] attributeArray)
        {
            List<int> extra = new List<int>();
            ALCContextAttributes attributes = new ALCContextAttributes();

            void ParseAttribute(int @enum, int value)
            {
                switch (@enum)
                {
                    case (int)ALC.ContextAttribute.Frequency:
                        attributes.Frequency = value;
                        break;
                    case (int)ALC.ContextAttribute.MonoSources:
                        attributes.MonoSources = value;
                        break;
                    case (int)ALC.ContextAttribute.StereoSources:
                        attributes.StereoSources = value;
                        break;
                    case (int)ALC.ContextAttribute.Refresh:
                        attributes.Refresh = value;
                        break;
                    case (int)ALC.ContextAttribute.Sync:
